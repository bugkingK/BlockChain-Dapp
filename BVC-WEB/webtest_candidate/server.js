...
app.get('/candidatelist', function(req, res){
    fs.readFile('candidatelist.html', 'utf8', function(err, data){
        dbFunc.SelectCandidateList();
    })
})

// 2. 후보자를 등록합니다.
app.get('/setCandidate/:result', function (req, res) {
    var candidate=req.params.result;
    var candidateinfo=candidate.split(",");
    
    blockFunc.setCandidate(0, function(candidateid){
        console.log(candidateid)
        dbFunc.InsertCandidateInfo(candidateid, candidateinfo, function(result){
            res.send(result);
        })
    })
});
